...
export function Metric({ label, value, detail, tone = 'default' }: { label: string; value: string; detail?: string; tone?: 'default' | 'positive' | 'negative' }) { return <div><p className="font-mono text-[9px] uppercase tracking-[.19em] text-muted-foreground">{label}</p><p className={`mt-2 font-display text-3xl ${tone === 'positive' ? 'text-primary' : tone === 'negative' ? 'text-destructive' : 'text-accent'}`} data-testid={`metric-${label.toLowerCase().replaceAll(' ', '-')}`}>{value}</p>{detail && <p className="mt-1 text-xs text-muted-foreground">{detail}</p>}</div>; }
export function Sparkline({ values = [], positive = true }: { values?: number[]; positive?: boolean }) { if (!values.length) return <div className="h-8 w-20 rounded skeleton" />; const min = Math.min(...values); const max = Math.max(...values); const span = max - min || 1; const points = values.map((v, i) => `${(i / (values.length - 1 || 1)) * 100},${28 - ((v - min) / span) * 24}`).join(' '); return <svg viewBox="0 0 100 30" className={`h-8 w-24 ${positive ? 'text-primary' : 'text-destructive'}`} fill="none" aria-label="Trend sparkline"><polyline points={points} vectorEffect="non-scaling-stroke" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>; }
